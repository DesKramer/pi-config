...
const MAX_VISIBLE_TOOL_TRAIL = 5;
...
function renderAgentProgress(
	...
	// (live subagent activity) render inline beneath each row. Keep this as a
	// compact live trail: show only the latest few calls so long scout runs do
	// not fill the whole screen with historical tool rows. The header still shows
	// the total tool count.
	const visibleTools = prog.recentTools.slice(-MAX_VISIBLE_TOOL_TRAIL);
