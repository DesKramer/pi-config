...
- CDP is separate and gives local processes that can reach its loopback port powerful page access. `launch_browser` uses the dedicated `user_data_dir` when configured and otherwise a fresh non-default temporary profile, matching modern Chrome's [remote-debugging security policy](https://developer.chrome.com/blog/remote-debugging-port), rather than the user's normal profile.
