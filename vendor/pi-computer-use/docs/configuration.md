...
Sets a dedicated persistent Chromium profile for `launch_browser`. `~` is expanded to the current user's home directory, absolute paths are normalized, and relative paths are resolved from Pi's current working directory. The directory is created only when `launch_browser` is invoked. Do not point this at a normal interactive Chrome profile. When unset, every launch keeps the original temporary-profile behavior.
...
PI_COMPUTER_USE_USER_DATA_DIR=~/.pi/browser-profiles/agent
...
Use a dedicated, non-default profile with `--user-data-dir=<directory>`. Chrome 136 and later ignore remote-debugging switches for the default data directory as a security measure. `launch_browser` uses the configured `user_data_dir`, or creates a separate temporary CDP profile when it is unset, and binds discovery to a randomly allocated loopback port.
