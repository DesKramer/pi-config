...
import { resolveUserDataDir } from "./browser-launch.ts";
...
function normalizePartial(raw: unknown): Partial<ComputerUseConfig> {
	...
	const userDataDir = (source as any).user_data_dir;
	if (typeof userDataDir === "string" && userDataDir.trim()) out.user_data_dir = userDataDir.trim();
	return out;
}
...
function readEnv(): Partial<ComputerUseConfig> {
	...
	if (managedBrowser === "helium" || managedBrowser === "chrome") out.managed_browser = managedBrowser;
	if (process.env.PI_COMPUTER_USE_USER_DATA_DIR?.trim()) out.user_data_dir = process.env.PI_COMPUTER_USE_USER_DATA_DIR.trim();
	return out;
}
...
export function loadComputerUseConfig(cwd: string): LoadedComputerUseConfig {
	...
	config.user_data_dir = resolveUserDataDir(config.user_data_dir, cwd);
	activeConfig = config;
